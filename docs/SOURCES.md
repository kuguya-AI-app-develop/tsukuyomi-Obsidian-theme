...
v0.7.1 结合用户选择的月读城市与八千代舞台。创作者陈述、制作图和官网场景图分别提供空间方向与形态参考；下文的视觉观察不是官方设计规范。
...
- [官网彩叶狐狸装扮立绘](https://www.cho-kaguyahime.com/assets/img/character/character_1main_3.png)：官方角色页链接的正面装扮图，是 v0.7.1 狐狸头重绘的主要依据。观察到宽阔杏色额头、细窄闭眼、奶油色双瓣下脸与短吻、深色三角鼻，以及带浅粉内侧和内向奶油色毛簇的耳朵。这是本项目对单张立绘的观察，不是官方三视图设定稿。
- 用户提供的彩叶狐狸装扮截图：补充角度参考；主题仅绘制头部，不另取官方吉祥物名称。参考图片均不进入安装产物。
...
以上参考图片没有打包进主题。城市、舞台和鱼由本项目重新设计为几何场景；DOGE、FUSHI 与彩叶狐狸装扮头部依据官方角色图制作矢量二创，狐狸另参考用户截图的角度，粉色伙伴依据用户截图重绘。四位伙伴分散在空白页上方、游鱼保留在下方两侧，是本主题的布局取舍；具体色值、组件形状与信息层级也由本项目决定，不是官方界面规范或像素级复刻。场景参考如何落到阅读界面，见 [DESIGN.md](DESIGN.md)。

[狐狸头参考与重绘对照](FOX-REFERENCE.html) 并列展示官网参考图与本项目矢量重绘，供核对外形；它是研究文档，不进入主题安装产物。
